...
const scrapingProfile = ()=>{
    ...
    wait(2000).then(function(){
        ...
        const dataInline = function(element,string){
            const selector = element.querySelector(string)
            return  selector? selector.innerText:'';
        };

        const element = document.querySelector("div.ph5.pb5 > div.display-flex.mt2 ")
        data.personal.name = dataInline(element,"ul li")
        data.personal.title = dataInline(element,"h2")
        data.personal.location = dataInline(element,"ul.pv-top-card--list-bullet li")
        ...
        for(let i=0; i<education.length; i++){
             const element =  education[i].querySelector('a > .pv-entity__summary-info ');
             const company = dataInline(element,'h3');
             const level = dataInline(element,'p span.pv-entity__comma-item');
